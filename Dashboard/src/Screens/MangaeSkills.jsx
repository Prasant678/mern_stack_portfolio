...
import { clearAllSkillErrors, deleteSkill, getAllSkills, resetSkillSlice, updateSkill } from "@/store/Slice/skillSlice";
        ...
        <TabsContent value="week">
          <Card className="bg-gradient-to-r from-slate-900 to-slate-700 text-white">
            ...
            <CardContent className="grid sm:grid-cols-2 gap-4">
              {skills.map((element) => {
                return (
                  <Card key={element._id} className="bg-inherit">
                    <CardHeader className="text-2xl text-white font-bold flex items-center justify-between flex-row">
                      {element.title}
                      <TooltipProvider>
                        <Tooltip>
                    ...
                    <CardFooter>
                      <Label className="text-xl text-white mr-2">Proficiency:</Label>
                      <Input
                        type="number"
